...
export default function AdminPage() {
  ...
  const addFurniture = () => {
    if (!preview)    return flash('⚠ Please upload a 2D furniture image')
    if (!model3d)    return flash('⚠ Please upload a 3D model (.glb or .gltf)')
    if (!form.name)  return flash('⚠ Please enter a furniture name')
    const item = {
      id:`f_${Date.now()}`, ...form,
  ...
  const ItemCard = ({ item, section, isTexture }) => (
    <div className="admin-item-card">
      ...
      <button className="admin-item-card__delete" onClick={() => del(section, item.id)}>✕</button>
    </div>
  )
        ...
        <div className="admin-form-card">
          ...
          <div style={{ marginBottom:14 }}>
            <div style={{ fontSize:'0.7rem', fontWeight:700, color: preview ? '#43d9ad' : '#9b95ff', textTransform:'uppercase', letterSpacing:'0.08em', marginBottom:6, display:'flex', alignItems:'center', gap:6 }}>
              <span style={{ width:16, height:16, borderRadius:'50%', background: preview ? '#43d9ad' : 'rgba(108,99,255,0.5)', display:'inline-flex', alignItems:'center', justifyContent:'center', fontSize:9, color:'#fff', flexShrink:0 }}>{preview ? '✓' : '1'}</span>
              2D Image (required)
            </div>
            <UploadBox preview={preview} onFile={setPreview} onClear={() => setPreview(null)} icon="🖼" label="Upload furniture image (.png / .jpg)" />
          </div>
          ...
          <div style={{ marginBottom:14 }}>
            <div style={{ fontSize:'0.7rem', fontWeight:700, color: model3d ? '#43d9ad' : '#9b95ff', textTransform:'uppercase', letterSpacing:'0.08em', marginBottom:6, display:'flex', alignItems:'center', gap:6 }}>
              <span style={{ width:16, height:16, borderRadius:'50%', background: model3d ? '#43d9ad' : 'rgba(108,99,255,0.5)', display:'inline-flex', alignItems:'center', justifyContent:'center', fontSize:9, color:'#fff', flexShrink:0 }}>{model3d ? '✓' : '2'}</span>
            ...
            <label style={{ display:'flex', alignItems:'center', gap:10, padding:'12px', borderRadius:10, border:`1px solid ${model3d ? 'rgba(67,217,173,0.5)' : 'rgba(255,255,255,0.1)'}`, background: model3d ? 'rgba(67,217,173,0.06)' : 'rgba(255,255,255,0.03)', cursor:'pointer', transition:'all .15s' }}>
              ...
              {model3d && (
                <button onClick={e => { e.preventDefault(); setModel3d(null); setModel3dName('') }}
                  style={{ fontSize:10, color:'#ff6b6b', background:'none', border:'none', cursor:'pointer', flexShrink:0 }}>✕ Remove</button>
              )}
          ...
          <div style={{ display:'flex', gap:8, marginBottom:12, padding:'8px 10px', borderRadius:8, background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.06)' }}>
            <span style={{ fontSize:10, color: preview ? '#43d9ad' : '#555' }}>{preview ? '✓' : '○'} 2D Image</span>
            <span style={{ color:'#333' }}>·</span>
            <span style={{ fontSize:10, color: model3d ? '#43d9ad' : '#555' }}>{model3d ? '✓' : '○'} 3D Model</span>
            <span style={{ color:'#333' }}>·</span>
            <span style={{ fontSize:10, color: form.name ? '#43d9ad' : '#555' }}>{form.name ? '✓' : '○'} Name</span>
          </div>
  ...
  const TexturesView = () => (
    <div>
      ...
      <div className="admin-page-grid">
        <div className="admin-form-card">
          <h3>+ Add Texture</h3>
          <UploadBox preview={texPrev} onFile={setTexPrev} onClear={() => setTexPrev(null)} icon="🎨" label="Upload texture image" />
